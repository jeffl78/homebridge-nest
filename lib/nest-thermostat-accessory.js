...
function NestThermostatAccessory(conn, log, device, structure) {
	...
	// Setting up our humidity sensor
	var bindCharacteristicHumidity = function (characteristic, desc, getFunc, setFunc, format) {
		this.bindCharacteristic(humiditySensor, characteristic, desc, getFunc, setFunc, format);
	}.bind(this);

	bindCharacteristicHumidity(Characteristic.CurrentRelativeHumidity, "Current humidity", this.getCurrentRelativeHumidity, null, function(val) {
		return val + "%";
	});

...
function NestThermostatAccessory(conn, log, device, structure) {
	...
	var humiditySensor = this.addService(Service.HumiditySensor);
	var temperatureSensor = this.addService(Service.TemperatureSensor);
	...
	// Set up characteristics for Thermostat
	...
	// Set up characterists for Humidity Sensor
	this.bindCharacteristic(humiditySensor, Characteristic.CurrentRelativeHumidity, "Current humidity", this.getCurrentRelativeHumidity, null, function(val) {
		return val + "%";
	});

	// Set up characteristics for Temperature Sensor
	this.bindCharacteristic(temperatureSensor, Characteristic.CurrentTemperature, "Current temperature", this.getCurrentTemperature, null, formatAsDisplayTemperature);
